...
export const ProjectsList = ({ projects, currentPage, totalItems, loadNextPage, pageSize }) => {
  ...
  const isStaff = config.user.is_staff === "True";
  
  return (
    <>
      <Elem name="list">
        {projects.map((project) => (
          <ProjectCard key={project.id} project={project} isStaff={isStaff} />
        ))}
      </Elem>
      <Elem name="pages">
        <Pagination
  ...
  return (
    <Block name="empty-projects-page">
      ...
      {isStaff ? (
        ...
        <p>Contact OLLI Staff to receive projects</p>
      )}
    </Block>
  );
};

const ProjectCard = ({ project, isStaff }) => {
  const color = useMemo(() => {
