...
class ProjectListAPI(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        projects = Project.objects.all() if self.request.user.is_staff else Project.objects.filter(task_of=self.request.user)
...
class ProjectAPI(generics.RetrieveUpdateDestroyAPIView):
    ...
    def patch(self, request, *args, **kwargs):
        task = request.data.get('task_of_id')
        if task:
            task_of_id = request.data.get('task_of_id')
            current_project = self.get_object()
            current_project.task_of_id = task_of_id
